...
/// Liste des arrêts d'une ligne — 1 section "aller" + 1 section "retour",
/// chacune en colonne verticale avec son propre tracé coloré et ses arrêts
/// en ordre. Les terminus de chaque sens servent de header.
///
/// Le schéma topologique avancé (trunk + rectangle de boucle) a été retiré
/// au profit de cette représentation symétrique, plus simple et cohérente
/// pour toutes les topologies (linéaire ou circulaire).
...
/// Affiche aller et retour d'une ligne en 2 sections empilées, chacune
/// avec son propre tracé coloré et ses arrêts en ordre. Header = nom du
/// terminus de chaque sens, fallback "Aller" / "Retour" si vide.
///
/// Lit directement depuis `getLineGroup` pour avoir aller et retour quel
/// que soit le résultat de [PublicTransportService.lineBranchesFor]
/// (qui peut renvoyer linéaire avec aller-only en `mainBranch`).
class _LegacyBranchesView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final allerStops = group?.aller?.stops
            .map((s) => BranchStop(name: s.name, position: s.position))
            .toList() ??
        const <BranchStop>[];
    final retourStops = group?.retour?.stops
            .map((s) => BranchStop(name: s.name, position: s.position))
            .toList() ??
        const <BranchStop>[];

    String terminusName(List<BranchStop> stops) {
      if (stops.isEmpty) return '';
      return stops.last.name.trim();
    }

    final allerTerm = terminusName(allerStops);
    final retourTerm = terminusName(retourStops);
    final allerHeader = allerTerm.isNotEmpty
        ? allerTerm
        : TransitStrings.t('branch.aller', locale);
    final retourHeader = retourTerm.isNotEmpty
        ? retourTerm
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          if (allerStops.isNotEmpty) ...[
            _BranchHeader(
                label: '$towardLabel $allerHeader', color: color),
            const SizedBox(height: 2),
            _BranchView(stops: allerStops, color: color),
          ],
          if (retourStops.isNotEmpty) ...[
            const SizedBox(height: 14),
            _BranchHeader(
                label: '$towardLabel $retourHeader', color: color),
            const SizedBox(height: 2),
            _BranchView(stops: retourStops, color: color),
          ],
        ],
      ),
    );
